...
## TC-08 图片编辑（Edit Model）

**目的：** 验证图片编辑功能，通过文本指令修改已有图片。

**请求：**

```bash
# 首先准备一张测试图片的 base64 编码
# 假设有一张 cat.jpg，编码方式：
# base64 -i cat.jpg | tr -d '\n' > cat.b64

curl -s -X POST http://localhost:8080/v1beta/models/gemini-3.1-flash-image-edit:generateContent \
  -H "Content-Type: application/json" \
  -H "x-goog-api-key: $KIEAI_API_KEY" \
  -d '{
    "contents": [{
      "parts": [
        ...
        {"inlineData": {"mimeType": "image/jpeg", "data": "'$(cat cat.b64)'"}}
      ]
    }],
    "generationConfig": {
      "imageConfig": {
...
| HTTP 状态码 | `200 OK` |
| `candidates` 数组 | 长度 ≥ 1 |
| `candidates[0].content.parts` | 包含编辑后的图片 |
| `inlineData.mimeType` | `"image/png"` |
| `finishReason` | `"STOP"` |
...
1. **无图片输入（应失败）：**
   ...
   curl -s -X POST http://localhost:8080/v1beta/models/gemini-3.1-flash-image-edit:generateContent \
     -H "Content-Type: application/json" \
     -H "x-goog-api-key: $KIEAI_API_KEY" \
     -d '{"contents": [{"parts": [{"text": "Add a hat"}]}]}'
   ```
   期望：HTTP 400，错误消息包含 "requires at least one image"

2. **Prompt 过长（>5000 字符，应失败）：**
   期望：HTTP 400，错误消息包含 "exceeds 5000 characters"

3. **图片过多（>10 张，应失败）：**
   期望：HTTP 400，错误消息包含 "too many images"

**注意事项：**
- Edit 模型使用 `image_urls` 参数，图片是必需的
- 最多支持 10 张图片（text-to-image 模型支持 14 张）
- Prompt 最长 5000 字符（text-to-image 模型支持 20000 字符）
- Edit 模型仅使用 `aspectRatio` 参数，不支持 `imageSize`（分辨率）参数
